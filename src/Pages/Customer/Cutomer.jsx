...
import { BiEdit,BiTrashAlt,BiGroup,BiBookBookmark } from 'react-icons/bi';
import classess from './Customer.module.css';
...
const Customer = () =>{
    ...
    const [showAgigng,setShowAging] = useState(false);
    const [customerRecords,setCustomerRecords] = useState([])

    useEffect(()=>{ 
        sendRequest({url:ctx.url_base+`/api/customerPG?page=${currentPage-1}&size=${PG.SIZE}`},getData); 
    ...
    const getCustomerRecords = (form)=>{ 
        // console.log(form)
        const formExport = {url:ctx.url_base+'/api/getDistCust', 
            method: 'Post',
            headers: {'Content-Type': 'application/json',},
            form,
            }; 
            sendRequest(formExport,(data)=>{setCustomerRecords(data)});
    }
    let item =  itemsState.map((item)=>{
        return (<Items key={item.id}>
                    ...
                    <div className={classess.cust}>
                        <button onClick={()=>{setShowAging(true); getCustomerRecords(item)}}><Card><BiBookBookmark/></Card></button>
    ...
    return(
        <div>
            ...
            {
                showAgigng &&
                <Modal withSize="notNull" onClose={(val)=>{setShowAging(val)}} >
                     <Aging
                        List = {List}
